...
export default function MfaVerificationDialog({ mfaToken, onVerified, onCancel }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Use axios instance so base URL is correct; backend returns { accessToken, refreshToken }
            const resp = await api.post(`/auth/mfa/verify-login`, { mfaToken, code });
            if (resp && resp.status === 200) {
                const data = resp.data;
                onVerified(data.accessToken, data.refreshToken);
            } else {
                setError("Invalid MFA response from server");
            }
        } catch (err) {
            ...
            if (err?.response?.data) {
                ...
                setError(d.error || d.message || JSON.stringify(d));
            } else if (err?.message) {
