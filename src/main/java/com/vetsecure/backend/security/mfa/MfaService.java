...
public class MfaService {
    ...
    /** Generate the current valid TOTP code for testing purposes */
    public String generateCurrentTotp(String base32Secret) {
        try {
            var codeGenerator = new DefaultCodeGenerator(HashingAlgorithm.SHA1);
            var timeProvider = new SystemTimeProvider();
            long currentBucket = Math.floorDiv(timeProvider.getTime(), 30);
            return codeGenerator.generate(base32Secret, currentBucket);
